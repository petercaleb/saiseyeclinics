        ...
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>{{ $clinic->clinic }}</h1>
                    <small>
                        {{ $schedule->day }} || {{ format_date($schedule->date) }} ||
                        {{ format_date($schedule->time, 'H:i A') }}
                    </small>
                </div>
                <div class="col-sm-6">
                        ...
                        <div class="card-body">
                            <div class="tab-content">
                                ...
                                <div class="tab-pane" id="treatmentTab">
                                    @include('users.schedules.partials.treatment_options')
    ...
    @if ($diagnosis)
        <script>
            $(document).ready(function() {
                ...
                $('.lensPowerTitle').html("Lens Power 1")
                $('.lensPrescriptionTitle').html("Lens Prescription 1")
                ...
                //Treatment options
                $('.treatmentOption').on('click', function(e) {
                    e.preventDefault();
                    var treatmentOption = $(this).data('option');
                    switch (treatmentOption) {
                        case "Treatment 1":
                            resolveTreatmentOption1()
                            break;
                        case "Treatment 2":
                            resolveTreatmentOption2();
                            break;
                    }
                    return;
                })

                function resolveTreatmentOption1() {
                    title = {
                        power: "Lens Power 1",
                        prescription: "Lens Prescription 1",
                        prescriptionButton: "Add Lens Prescription 1",
                        powerButton: "Edit lens power 1",
                        type: "Treatment 1"
                    };
                    resolveTreatmentOptionTitle(title);
                }

                function resolveTreatmentOption2() {
                    title = {
                        prescription: "Lens Prescription 2",
                        prescriptionButton: "Add Lens Prescription 2",
                        powerButton: "Edit lens power 2",
                        type: "Treatment 2"
                    };
                    resolveTreatmentOptionTitle(title);
                }

                function verifyTreatmentOptionKey(title) {
                    let flag = false;
                    try {
                        let allowedKeys = ["power", "prescription", "type", "prescriptionButton", "powerButton"];
                ...
                function resolveTreatmentOptionTitle(title = {}) {
                    try {
                        if (typeof title === "object") {
                            if (verifyTreatmentOptionKey(title)) {
                                $('.lensPowerTitle').html(title.type)
                                $('.lensPrescriptionTitle').html(title.type)
                                $('.newLensPrescriptionBtn').html(title.prescriptionButton);
                                $('.editLensPowerBtn').html(title.powerButton);
                                $('input[name="form_type"]').val(title.type);
                                return true;
                            }
                        } else {
                ...
                $(document).on('click', '.viewLensPrescription', function(e) {
                    ...
                    toggleVisibility("lensPrescriptionPreamble", "show");
                    $(".treatmentOptionTitle").html("Lens Prescription");
                    $.ajax({
                        url: path,
                        type: 'POST',
                        data: {
