...
class PaymentsBillController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($data['frame_amount'] == 0 && $data['lens_amount'] == 0) {
        ...
        if ($payments_details->client_type->type == 'Cash') {
            ...
            $payment_bill->frame_amount = $data['frame_amount'];
            $payment_bill->lens_amount = $data['lens_amount'];
            $payment_bill->agreed_amount = $payment_bill->frame_amount + $payment_bill->lens_amount;
        } else {
            if ($data['frame_amount'] == 0 && $data['lens_amount'] == 0) {
                $payment_bill->bill_status = 'PENDING';
            } else {
        ...
        if ($payment_bill->save()) {
            ...
            $report->update([
                ...
                'frame_amount' => $payment_bill->frame_amount,
                'lens_amount' => $payment_bill->lens_amount,
            ]);
